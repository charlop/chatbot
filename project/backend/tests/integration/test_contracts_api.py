...
class TestContractSearchEndpoint:
    ...
    async def test_search_contract_found_in_database(
        ...
        contract = ContractFactory.build(
            account_number="ACC-TEST-12345", contract_id="GAP-2024-0001"
        )
        ...
        response = await async_client.post(
            "/api/v1/contracts/search", json={"account_number": "ACC-TEST-12345"}
        )
        ...
        assert data["account_number"] == "ACC-TEST-12345"
    ...
    async def test_search_contract_not_found(
        ...
        response = await async_client.post(
            "/api/v1/contracts/search", json={"account_number": "NONEXISTENT"}
        )
    ...
    async def test_search_contract_whitespace_trimming(
        ...
        # Create a test contract
        contract = ContractFactory.build(
            account_number="ACC-TEST-99999", contract_id="GAP-2024-0099"
        )
        db_session.add(contract)
        await db_session.commit()
        ...
        response = await async_client.post(
            ...
            json={"account_number": "  ACC-TEST-99999  "},
        )
...
class TestContractAuditLogging:
    ...
    async def test_search_creates_audit_event(
        ...
        # Create a test contract
        contract = ContractFactory.build(
            account_number="ACC-AUDIT-TEST", contract_id="GAP-2024-AUDIT"
        )
        db_session.add(contract)
        await db_session.commit()
        ...
        # Search for the contract
        response = await async_client.post(
            "/api/v1/contracts/search", json={"account_number": "ACC-AUDIT-TEST"}
        )
        ...
        assert audit_event.event_data["account_number"] == "ACC-AUDIT-TEST"
